...
void connectAWS()
{
  ...
  Serial.println("=====================\n\n");
  uint64_t AWSconnectTime = esp_timer_get_time();
  while (!client.connect(THINGNAME)) {
    if(esp_timer_get_time() - AWSconnectTime >= 30000000) {
      ESP.restart();
      }
    Serial.print(".");
    delay(100);
...
void grabImage(){
  ...
  if(fb != NULL && fb->format == PIXFORMAT_JPEG && fb->len < bufferSize){
    ...
    if(!result){
      Serial.println("Publishing error. Restart device.");
      ESP.restart();
    }
  }
